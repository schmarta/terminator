...
$Flag = Join-Path $ClaudeDir ".terminator-active"
...
$Valid = @('off','lite','t-800','t-1000')
if (-not ($Valid -contains $Mode)) { exit 0 }

$Esc = [char]27
if ([string]::IsNullOrEmpty($Mode) -or $Mode -eq "t-800") {
    [Console]::Write("${Esc}[38;5;196m[TERMINATOR]${Esc}[0m")
} else {
    $Suffix = $Mode.ToUpperInvariant()
    [Console]::Write("${Esc}[38;5;196m[TERMINATOR:$Suffix]${Esc}[0m")
}
