#!/usr/bin/env node
// terminator — shared configuration resolver
//
// Resolution order for default model:
//   1. TERMINATOR_DEFAULT_MODE environment variable
//   2. Config file defaultMode field:
//      - $XDG_CONFIG_HOME/terminator/config.json (any platform, if set)
//      - ~/.config/terminator/config.json (macOS / Linux fallback)
//      - %APPDATA%\terminator\config.json (Windows fallback)
//   3. 't-800'
...
// T-model intensity levels. Internally these map to terminator-style behavior tiers
// (lite → light, t-800 → default/full, t-1000 → max compression).
const VALID_MODES = ['off', 'lite', 't-800', 't-1000'];

function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'terminator');
  }
  ...
  return path.join(os.homedir(), '.config', 'terminator');
}
...
function getDefaultMode() {
  ...
  const envMode = process.env.TERMINATOR_DEFAULT_MODE;
...
// flag path (~/.claude/.terminator-active) with a symlink to clobber other files.
//
...
//
// Set TERMINATOR_DEBUG=1 to emit stderr diagnostics when flag writes are refused.
//
// Silent-fails on any filesystem error — the flag is best-effort.
function safeWriteFlag(flagPath, content) {
  const debug = process.env.TERMINATOR_DEBUG === '1';
  try {
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (!realStat.isDirectory()) {
          if (debug) process.stderr.write(`[terminator] safeWriteFlag: symlink target ${realFlagDir} is not a directory\n`);
          return;
        }
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[terminator] safeWriteFlag: symlink target ${realFlagDir} owned by uid ${realStat.uid}, not current user ${process.getuid()}\n`);
            return;
          }
        } else {
          ...
          if (!normalizedReal.toLowerCase().startsWith(normalizedHome.toLowerCase() + path.sep) &&
            ...
            if (debug) process.stderr.write(`[terminator] safeWriteFlag: symlink target ${normalizedReal} is outside home directory ${normalizedHome}\n`);
            return;
          }
        }
      } else {
    ...
    const tempPath = path.join(realFlagDir, `.terminator-active.${process.pid}.${Date.now()}`);
...
//
// MAX_FLAG_BYTES is a hard cap. The longest legitimate value is "t-1000"
// (6 bytes); 64 leaves slack without enabling exfil.
...
// ($CLAUDE_CONFIG_DIR/.terminator-history.jsonl).
//
  ...
  try {
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[terminator] appendFlag: symlink target ${realDir} owned by uid ${realStat.uid}\n`);
            return;
          }
        } else {
          const home = os.homedir();
